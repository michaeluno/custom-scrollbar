...
(function ( $ ) {
    ...
    $.fn.aCustomScrollbar_AdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
     ...
     */  
    $( document ).bind( 'custom-scrollbar_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){          
        ...
        $.each( $.fn.aCustomScrollbar_AdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {                
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerCustomScrollbar_AdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     * 
     * Unlike the `custom-scrollbar_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).bind( 'custom-scrollbar_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aCustomScrollbar_AdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
    ...
    $.fn.registerCustomScrollbar_AdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aCustomScrollbar_AdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
