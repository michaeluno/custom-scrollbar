...
abstract class CustomScrollbar_AdminPageFramework_FormFieldset_Base extends CustomScrollbar_AdminPageFramework_WPUtility {
    ...
    private function _getFormatted($aField, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aField, $this->_getFieldTypeDefaultArguments($aField['type'], $aFieldTypeDefinitions) + CustomScrollbar_AdminPageFramework_Format_Fieldset::$aStructure);
    }
...
jQuery( document ).ready( function() {
    var _nodePositionIndicators = jQuery( '#{$sFieldsContainerID} .custom-scrollbar-field .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
        ...
        if ( ! jQuery( '#{$sFieldsContainerID} .custom-scrollbar-repeatable-field-buttons' ).length ) { 
            // Adds the buttons
            jQuery( '#{$sFieldsContainerID} .custom-scrollbar-field' ).prepend( $_sButtonsHTML ); 
        }
    }     
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='custom-scrollbar-sortable-field-enabler-script'>" . $_sScript . "</script>";
    }
}