...
jQuery( document ).ready( function(){
    ...
    jQuery().registerCustomScrollbar_AdminPageFrameworkCallbacks( {				
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            oSortedFields.children( '.custom-scrollbar-field' ).each( function( iIndex ) {
...
JAVASCRIPTS;
    ...
    protected function getStyles() {
        return <<<CSSRULES
/* Textarea Field Type */
.custom-scrollbar-field-textarea .custom-scrollbar-input-label-string {
    vertical-align: top;
    margin-top: 2px;
}     
/* Rich Text Editor */
.custom-scrollbar-field-textarea .wp-core-ui.wp-editor-wrap {
    margin-bottom: 0.5em;
}
.custom-scrollbar-field-textarea.custom-scrollbar-field .custom-scrollbar-input-label-container {
    vertical-align: top; 
} 
/* For meta-boxes */
.postbox .custom-scrollbar-field-textarea .custom-scrollbar-input-label-container {
    width: 100%;
}
CSSRULES;
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $_bIsArray), "<div class='custom-scrollbar-input-label-container {$_sClassSelector}'>", "<label for='" . $aField['attributes']['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $_bIsArray), $_sLabel ? "<span class='custom-scrollbar-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $_bIsArray), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $_bIsArray),);
        return implode('', $_aOutput);
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='custom-scrollbar-textarea-enabler'>" . $_sScript . "</script>";
    }
}