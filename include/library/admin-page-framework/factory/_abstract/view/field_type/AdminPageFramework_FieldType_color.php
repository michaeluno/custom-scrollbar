...
.custom-scrollbar-field-color .wp-picker-container {
    vertical-align: middle;
}
.custom-scrollbar-field-color .ui-widget-content {
    border: none;
    background: none;
    color: transparent;
}
.custom-scrollbar-field-color .ui-slider-vertical {
    width: inherit;
    height: auto;
    margin-top: -11px;
}
.custom-scrollbar-field-color .custom-scrollbar-field .custom-scrollbar-input-label-container {
    vertical-align: top; 
}
.custom-scrollbar-field-color .custom-scrollbar-repeatable-field-buttons {
    margin-top: 0;
}
CSSRULES;
...
jQuery( document ).ready( function(){
    ...
    jQuery().registerCustomScrollbar_AdminPageFrameworkCallbacks( {     
        added_repeatable_field: function( node, sFieldType, sFieldTagID, sCallType ) {
            ...
            registerCustomScrollbar_AdminPageFrameworkColorPickerField( nodeNewColorInput );     
...
JAVASCRIPTS;
    ...
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div class='custom-scrollbar-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span class='custom-scrollbar-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $aField['label'] . "</span>" : "") . "<input " . $this->getAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
    }
