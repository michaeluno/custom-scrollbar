...
abstract class CustomScrollbar_AdminPageFramework_Factory_View extends CustomScrollbar_AdminPageFramework_Factory_Model {
    ...
    private function _getSettingNotice(array $aNotice) {
        ...
        $aNotice['aAttributes']['class'] = $this->oUtil->getClassAttribute($this->oUtil->getElement($aNotice, array('aAttributes', 'class'), ''), 'custom-scrollbar-settings-notice-container', 'notice is-dismissible');
        return "<div " . $this->oUtil->getAttributes($aNotice['aAttributes']) . ">" . "<p class='custom-scrollbar-settings-notice-message'>" . $aNotice['sMessage'] . "</p>" . "</div>";
    }
    public function _replyToGetFieldOutput($aField) {
        $_oField = new CustomScrollbar_AdminPageFramework_FormFieldset($aField, $this->oProp->aOptions, $this->_getFieldErrors(), $this->oProp->aFieldTypeDefinitions, $this->oMsg, $this->oProp->aFieldCallbacks);
